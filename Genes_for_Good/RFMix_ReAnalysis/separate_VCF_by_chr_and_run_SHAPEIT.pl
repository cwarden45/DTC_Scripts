...
my $inputVCF="ALL.chip.omni_broad_sanger_combined.20140818.snps_frequencies_UNRELATED_plus_1child_PLUS_GFG_CW.vcf";
...
###run with separate code for chrX
for (my $i=0; $i < 22; $i++){
	my $VCF_IN = "$SHAPEIT_chr_folder/chr".$chr_short[$i]."_input.vcf";
	my $MAP = "$map_folder/genetic_map_".$chr_long[$i]."_combined_b37.txt";
